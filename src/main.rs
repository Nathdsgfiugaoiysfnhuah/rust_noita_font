...
fn main() {
    ...
    let mut high = 0;
    let mut low = 0;
	let space_width = font.rasterize(' ',32.0).0.advance_width.round() as i64;
    for i in 33..=126 {
    ...
    println!("{high}");
    low *= -1;
    width += pictures.len() + space_width as usize + 1;
...
<FontData>	
	...
	<QuadChar id=\"32\" offset_x=\"0\" offset_y=\"0\" rect_h=\"{bh}\" rect_w=\"{space_width}\" rect_x=\"0\" rect_y=\"0\" width=\"{space_width}\" ></QuadChar>
"
    )
    .to_owned();
	xoff = space_width + 1;
    for picture in pictures {
        ...
        xml.push_str(&format!("\t<QuadChar id=\"{char_id}\" offset_x=\"0\" offset_y=\"0\" rect_h=\"{bh}\" rect_w=\"{w}\" rect_x=\"{xoff}\" rect_y=\"0\" width=\"{w}\" ></QuadChar>\n"));
